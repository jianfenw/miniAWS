...
int main(int argc, char const *argv[])
{
	...
	if (!(backend_udp_sockfd = udp_port_socket())){
		fprintf(stderr, "Error: Server C failed to create UDP socket\n");
		exit(1);
	}
...
int udp_port_socket(){
	...
	if ( (rv = getaddrinfo(NULL, SERVER_C_PORT_NUM, &hints, &res)) != 0){
		fprintf(stderr, "Error: AWS getaddrinfo failed: %s\n", gai_strerror(rv));
		exit(1);
	}
	for (p = res; p != NULL; p = p->ai_next){
		if ((sockfd = socket(p->ai_family, p->ai_socktype, p->ai_protocol)) == -1){
			perror("Error: Server C UDP socket");
			continue;
		}
